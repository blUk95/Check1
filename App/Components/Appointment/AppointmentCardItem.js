...
gap:10,alignItems:'center'}}>
       ...
       <View>
            <Text style={{
                fontSize:16,
                fontFamily:'appfont-semi'
            }}>{appointment.attributes?.hospitals?.data.attributes.Name} Heart Clinic</Text>
              <View style={{display:'flex',flexDirection:'row',
    gap:5,alignItems:'center',marginTop:5}}>
            <Ionicons name="location" size={20} color={Colors.PRIMARY}/>
            <Text style={{fontFamily:'appfont',
        color:Colors.GRAY}}>{appointment?.attributes?.hospitals?.data.attributes.Address}</Text>
     </View>
        <View style={{display:'flex',flexDirection:'row',
    gap:5,alignItems:'center',marginTop:5}}>
            <Ionicons name="document-text" size={20} color={Colors.PRIMARY}/>
            <Text style={{fontFamily:'appfont',
        color:Colors.GRAY}}>Booking No. {appointment.id}</Text>
        </View>
        </View>
    </View>
    </View>
